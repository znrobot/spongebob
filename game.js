...
const MATCHMAKING_POOL_SIZE = 30;
...
function getMatchmakingCode(index) {
  return "rm" + String(index % MATCHMAKING_POOL_SIZE).padStart(2, "0");
}
...
function startRandomMatch() {
  document.querySelector("input[name='gameMode'][value='double']").checked = true;
  updateMultiplayerUi();

  cancelRandomMatch();
  matchmaking.active = true;

  randomMatchButton.textContent = "取消配对";
  roomCodeInput.value = "";
  setNetworkStatus("正在搜索对手...");
...
function cancelRandomMatch() {
  ...
  if (!multiplayer.conn?.open) {
    setNetworkStatus("双人联机需要两台设备打开此网页：一人创建房间，另一人输入房间码加入。");
  }
  updateMultiplayerUi();
}
...
function handleNetworkMessage(message) {
  ...
  if (message.type === "ready") {
    ...
    if (message.matchmaking && isNetworkHost() && multiplayer.remoteReady && multiplayer._matchmakingHost) {
      multiplayer.localReady = true;
      sendNetworkMessage({ type: "ready", ready: true });
      setNetworkStatus("双方已准备，开始游戏！");
      updateMultiplayerUi();
      resetGame();
      return;
    }
...
document.querySelectorAll("input[name='gameMode']").forEach((input) => {
  input.addEventListener("change", () => {
    if (input.value === "single" && matchmaking.active) {
      cancelRandomMatch();
    }
    updateMultiplayerUi();
  });
